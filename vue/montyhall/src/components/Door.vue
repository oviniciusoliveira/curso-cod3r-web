<template>
  <div class="door-area">
    <div class="door-frame" :class="{ selected: selected && !open }">
      <Gift v-if="open && hasGift" />
    </div>
    <div class="door" :class="{ open }" @click="selected = !selected">
      <div class="number" :class="{ selected }">{{ number }}</div>
      <div class="knob" :class="{ selected }" @click="open = true"></div>
    </div>
  </div>
</template>

<script>
import Gift from "./Gift";
export default {
  name: "Door",
  components: { Gift },
  // Propriedades não são alteradas durante a execução do programa
  props: {
    number: {},
    hasGift: { type: Boolean },
  },
  data: function() {
    return {
      open: false,
      selected: false,
    };
  },
};
</script>

<style>
:root {
  --door-border-color: #441100;
  --selected-border-color: #bbbb00;
  --selected-border: 5px solid var(--selected-border-color);
  --door-border: 5px solid var(--door-border-color);
  --door-color: #773311;
}

.door-area {
  position: relative;
  width: 200px;
  height: 310px;
  border-bottom: 10px solid #aaaaaa;
  margin-bottom: 20px;
  font-size: 3rem;

  display: flex;
  justify-content: center;
}

.door-frame {
  position: absolute;
  height: 300px;
  width: 180px;

  border-left: var(--door-border);
  border-top: var(--door-border);
  border-right: var(--door-border);

  display: flex;
  justify-content: center;
  align-items: flex-end;
}

.door {
  position: absolute;
  top: 5px;
  height: 295px;
  width: 170px;
  background-color: var(--door-color);

  display: flex;
  flex-direction: column;
  align-items: center;
  padding: 10px;
}

.door .knob {
  height: 20px;
  width: 20px;
  border-radius: 10px;
  background-color: var(--door-border-color);
  align-self: flex-start;
  margin-top: 60px;
}

.door-frame.selected {
  border-left: var(--selected-border);
  border-top: var(--selected-border);
  border-right: var(--selected-border);
}
.door .number.selected {
  color: var(--selected-border-color);
}
.door .knob.selected {
  background-color: var(--selected-border-color);
}
.door.open {
    background-color: #00000077;
}
.door.open .knob {
    display: none;
}
.door.open .number {
    display: none;
}
</style>
